...
> - **Latencia del tracker ligero:** 0.143 ms/frame en la Orin, 350× de margen sobre su presupuesto de control de 50 ms (`P3-T4a-tracker-cost`). **No es el coste por fotograma del sistema:** el arrastre SAM2 domina, y a la resolución desplegada (**640**, el codo medido en EXP-1) son **174 ms** por paso — 5.76 Hz (`grounding.contract.CARRY_IMAGE_SIZE`/`CARRY_HZ`, dueños únicos del valor desde R-46). A 1024, el repliegue manual para blancos pequeños o lejanos, son 427 ms (2.34 Hz) según EXP-1, o 372 ms (2.69 Hz) según la medida anterior R-16 (`P4-R16-carry-rate-1024`; `P3-E1-TRT-fps` queda superada). Es ese número, no el del tracker, el que fija la cadencia.
...
> - **Bucle cerrado, con el copter volando su propia salida de control:** la entrega warm-start engancha **23/25** frente a **2/25** de la adquisición fría bloqueante, sobre 25 semillas CARLA independientes, McNemar exacto b=21, c=0, **p = 9.5e-07** (1.9e-06 tras Holm por Parte, 3.8e-05 en familia global; n_eff = 25, sin deflactar), y sobrevive a Holm en las dos familias (`P6.2-DELIVERY-warm-vs-cold-closedloop`). Lo que le cuesta al brazo frío **no** es que el objetivo salga de cuadro —no sale en ninguna semilla— sino la obsolescencia: la caja llega tras el retardo bloqueante de ~4.85 s y para entonces ya no cae sobre el objetivo. El alcance va con la cifra: el grounding se mantuvo constante mediante **designación por oráculo**, porque el Q8_0 desplegado no es discriminativo en nadir a 45 m, así que lo que se afirma es **acoplamiento de control condicionado a una designación correcta**, no grounding y entrega conjuntamente. Medido en la **RTX 3090**, la que corre el renderizador CARLA; la Jetson quedó ociosa en esta ejecución, y el arrastre SAM2 va topado a la tasa medida en la placa (2.69 Hz).
> - **La misma costura, corriendo en el dispositivo (cualitativo, fuera de la familia de Holm):** la demostración P6.2-SHOWCASE enruta el arrastre desplegado **literalmente a la Orin**: mantiene **24/24** pasos con IoU ≥ 0.25 y mediana **0.92** contra la verdad-terreno, y en vuelo en bucle cerrado —arrastre en la placa, control y renderizado en la 3090— alcanza paridad con el gemelo de la 3090 en **0.960** de IoU mediana por paso, con cobertura 0.495. No lleva puerta pre-registrada ni prueba estadística y **no** está en el registro de afirmaciones: enseña que la ruta medida en el rig se sostiene sobre el hardware de despliegue, y se cita como demostración, no como resultado contrastado ([`experiments/2026-07-24-p62-showcase/`](experiments/2026-07-24-p62-showcase/README.md)).
