...
- **Part V — v5 anticipatory grounding / warm-start acquire (PAUSED 2026-07-20 at P5.20, reopened once for P5.21 on 2026-07-24, resumable):** paused to start Part VI, with two open residuals carried forward rather than dropped — P5.19's grace precision is 2/4 (the wrong ones deliver a confident box instead of abstaining), and carry drift owns 8 of its 10 residual failures now that the capacity lever is dead (P5.20). **P5.21 carry lever = TIE [measured negative]** (run 2026-07-24 as Wave B of the Part VI program, real UAV123 imagery): the ROI-crop re-anchor does **not** beat plain SAM2 carry and mildly regresses it — plain 28/34 vs ROI 26/34, exact McNemar b=1/c=3, p=0.625, direction *against* ROI, with drift-reinforcement firing on `car10` (the crop is taken around an already-drifted box and the track is lost). Together with P5.15 (the carry is not the fragile part) and P5.20 (capacity is dead), that closes the last non-capacity carry lever; keep ROI-crop for **acquire prefill only**. The premise: the operator's prompt arrives mid-flight, not at frame 0 — the pre-prompt stream is free compute. Keep salient objects tracked over the idle window and select on command, instead of cold-acquiring under time pressure. Reframe: `experiments/PART5-PROPOSAL-anticipatory-grounding.md`. **Thesis-level scope, author-decided 2026-07-23 (R-28):** the thesis defends **maintain-and-deliver**, not select. Selecting among candidates is delivered as a measured proposal: R-16 shows the multi-candidate selector is OOM-killed at N=2 on the Orin with the deployed ring, and where memory did *not* bind (3090 replay) selection still failed on carry drift and referring-expression ambiguity — P5.20 gave a bigger SAM2 for free and recovered 0 cells, P5.4 cut acquire 4.9s to 2.08s and moved 0 cells. Do not restate Part V as "select works". **Statistical standing (added 2026-07-21, R-4/R-19):** the Part V verdicts below are as-run labels. Several are descriptively right but were never inferential at their n — P5.1 is p=0.125, the P5.3/P5.4/P5.5 gates were unreachable by construction, and the P5.10/P5.13/P5.17 "ties" have zero or one discordant pair, so no test ran. **P5.2 is the one that is properly powered** (p=6.10e-05 deflated to 23 independent clips, survives Holm; 3.05e-05 undeflated — cite the deflated one, per HANDOFF invariant I2) and is what the warm-start claim should rest on. Per-claim standing, with caveats, is in `thesis/claims.json` and `thesis/stats-report.md`; do not restate a verdict from this file without checking it there. **P5.1 warm-start acquire = YES [carry-bound]:** idle-window VLM seed + SAM2 catch-up + select-on-command lands 5/6 on the UAV123 `car*` clips, matches the GT-seed oracle exactly, beats the cold blocking acquire 1/6 — removes the ~135-frame delivery staleness that capped the Part IV arc. **P5.2 warm-start generalization = YES / speed-sweep NO:** 25 clips × 5 categories × a data-driven on-screen-speed sweep — WARM 21/25 vs COLD 5/25 (RQ-P5.2a YES, generalizes past cars), but the WARM−COLD gap is **flat in speed** (Spearman ρ=−0.06; RQ-P5.2b NO) — the warm-start win is delivery-lag removal, not motion-compensation, so cold fails broadly regardless of target speed.
- **Part VI — v6 closed-loop flight (IN PROGRESS):** every Part V number was measured on replayed video the system could not influence — no vehicle in the loop. Part VI puts the warm-start select in front of a flying copter so the pixels become a consequence of its own control output. Proposal: `experiments/PART6-PROPOSAL-closed-loop-flight.md`. The rig already exists (`runners/run_phase_c.py`: ArduCopter SITL as physics, a pose-slaved renderer, VLM→ByteTrack→PID→MAVLink closed) — the remaining work is porting the Part V select modules out of `experiments/` into it. Staging: **P6.0** flight-rig capability gate (PASS), **P6.1** renderer swap Gazebo→CARLA (YES), **P6.2** closed-loop select-and-follow vs an oracle-driven no-coupling control. Decisions recorded up front and still standing: do **not** adopt `ardupilot_gazebo` lockstep, and do **not** put the copter under CARLA physics — pose-slaving already delivers the ego-motion under test, and it is what made the renderer swappable. **P6.1 = YES:** CARLA 0.9.16 `Town10HD_Opt` renders at 48.1 Hz with 40 autonomous vehicles while the camera follows a live GUIDED flight (0→84.4 m at 60 m AGL); the control stack is untouched. That 48.1 Hz is **render-loop throughput with no perception in the window**, measured in sync mode where the sim ran 2.41x faster than wall clock — the "2.4x the control rate" headroom reading is withdrawn (R-10), and the renderer is position-slaved, not pose-slaved (yaw never arrived). This replaces the plan to upgrade the scene to the P5.9/P5.12 `select_arena` bank — the flight world was empty (four entities, no targets) and the vendored Gazebo asset library has no city or traffic. P6.1's non-gating G6 was recorded NOT RUN and first called a P6.2 blocker; **that was wrong and is corrected (2026-07-20T20:10Z)** — the deployed model is on the Jetson at `/home/jfdg/grounding/phase3-terse100eos-1024-q8_0.gguf` (+ `mmproj`), which is what `grounding/deploy/video.py` points at and what P5.17 itself grounded through via `JetsonBackend`. Only the merged HF/safetensors *training* directory is lost (costs LoRA resumption and re-export, not grounding). **P6.2 is not blocked.** **P6.2-DELIVERY = YES [oracle-designation scope]** — the flagship, and the one properly-powered Part VI number: with the copter flying its own control output and CARLA rendering async, closed-loop warm-start delivery beats the cold blocking acquire **WARM 23/25 vs COLD 2/25** (exact McNemar b=21, c=0, p=9.5e-07, which per-Part Holm raises to 1.9e-06 and the global family to 3.8e-05; **not deflated** — 25 independent CARLA seeds, so n_effective = n_rows = 25; `machine=rtx-3090` — the Jetson was idle for that run), and it survives Holm both per Part and globally. Cold's cost is **delivery staleness** (the ~4.85 s lag lands the box off-target), not the target exiting frame. Scope caveat: grounding was held constant via **ORACLE designation** (q8_0 is non-discriminative at 45 m nadir, G6), so the claim is about control-coupling **conditional on correct designation**, not about grounding+delivery jointly. **P6.2-COUPLING = BOUNDED NULL [gate ii]:** closing the control loop does **not** degrade the warm track — Wilcoxon two-sided p=0.596, median paired difference −0.42 px, CI [−4.56, +4.08] inside the warm-arm 6.70 px noise band; C1 closes as "warm carry survives self-induced ego-motion", never as proven equivalence. **P6.2-SHOWCASE = DONE, both halves:** the on-device carry seam held **24/24 at median IoU 0.92** literally on the Orin, and the closed-loop *flight* half flew with carry on the Orin over ssh-stdio — **median IoU 0.960 parity against the 3090 twin**, coverage 0.495. Two on-device follow-ups then mapped the resolution knobs: **EXP-1** located the carry-resolution elbow and adopted SAM2 track-res **640** as the measured default (**5.76 Hz vs 2.34 Hz at 1024** on the Orin, 99.4% of 1024's median IoU), keeping 1024 as a size-gated fallback for small/distant targets; **EXP-2** located the grounding-resolution elbow (a point-crop at 256 px out-grounds the whole frame at 1024) while the delivered-PASS verdict stayed a MISS at n=26. **Every Part VI result supports maintain-and-deliver, not select:** R-38 showed grounding is **not** the select bottleneck (same prompt frame, target 13/14 vs distractor 12/14, b=2/c=1, p=1.0 — symmetric, so the residual failure lives downstream in carry/delivery), and R-36 came back **NO [underpowered, scene-starved]** (b=5, c=0, p=0.0625 on the audit-clean n=14; select never wins a discordant pair). The R-28 author decision above stands unchanged.
