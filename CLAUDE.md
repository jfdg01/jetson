...
- **Part V — v5 anticipatory grounding / warm-start acquire (PAUSED 2026-07-20 at P5.20, resumable):** paused to start Part VI, with two open residuals carried forward rather than dropped — P5.19's grace precision is 2/4 (the wrong ones deliver a confident box instead of abstaining), and carry drift owns 8 of its 10 residual failures now that the capacity lever is dead (P5.20). The premise: the operator's prompt arrives mid-flight, not at frame 0 — the pre-prompt stream is free compute. Keep salient objects tracked over the idle window and select on command, instead of cold-acquiring under time pressure. Reframe: `experiments/PART5-PROPOSAL-anticipatory-grounding.md`. **P5.1 warm-start acquire = YES [carry-bound]:** idle-window VLM seed + SAM2 catch-up + select-on-command lands 5/6 on the UAV123 `car*` clips, matches the GT-seed oracle exactly, beats the cold blocking acquire 1/6 — removes the ~135-frame delivery staleness that capped the Part IV arc. **P5.2 warm-start generalization = YES / speed-sweep NO:** 25 clips × 5 categories × a data-driven on-screen-speed sweep — WARM 21/25 vs COLD 5/25 (RQ-P5.2a YES, generalizes past cars), but the WARM−COLD gap is **flat in speed** (Spearman ρ=−0.06; RQ-P5.2b NO) — the warm-start win is delivery-lag removal, not motion-compensation, so cold fails broadly regardless of target speed.
- **Part VI — v6 closed-loop flight (IN PROGRESS):** every Part V number was measured on replayed video the system could not influence — no vehicle in the loop. Part VI puts the warm-start select in front of a flying copter so the pixels become a consequence of its own control output. Proposal: `experiments/PART6-PROPOSAL-closed-loop-flight.md`. The rig already exists (`runners/run_phase_c.py` + `worlds/phase_c.sdf`: ArduCopter SITL as physics, Gazebo as a pose-slaved renderer, VLM→ByteTrack→PID→MAVLink closed) — the work is porting the Part V select modules out of `experiments/` into it and upgrading the scene to the P5.9/P5.12 `select_arena` bank. Staging: **P6.0** flight-rig capability gate, **P6.1** closed-loop select-and-follow vs an oracle-driven no-coupling control. Decision recorded up front: do **not** adopt `ardupilot_gazebo` lockstep — pose-slaving already delivers the ego-motion under test.
