"""P5.10 contract A/B select on the P5.9 sim scene bank (Part V).
...
Scene source: the P5.9 kerb-safe scene bank
(experiments/2026-07-17-kerbsafe-scenebank/runs/bank01..bank12), 240 frames @
...
Usage:
    ...
    .venv-ft/bin/python experiments/2026-07-17-simbank-select/select_p510.py --preflight
    .venv-ft/bin/python experiments/2026-07-17-simbank-select/select_p510.py \
        --matrix --out experiments/2026-07-17-simbank-select/runs
...
REPO = HERE.parents[1]
BANK = REPO / "experiments" / "2026-07-17-kerbsafe-scenebank" / "runs"
...
N_FRAMES = 240             # bank clip length (9.6 s)
T_P = 3.0                  # idle window seconds (clip-length bound; see README)
PROMPT_FRAME = round(T_P * FPS)          # 75
...
def load_bank_clip(clip: str, bank_root: Path = BANK):
    """Load one bank clip: frame paths + {oid: phrase} + {oid: [box|None]*240}.
    Asserts the P5.9 schema (2 objects, distinct phrases, 240 GT lines)."""
    d = bank_root / clip
...
def run_cell(clip: str, leg: str, phrases: dict, gtb: dict, tracks: dict,
    ...
    print(f"[P5.10 {clip}_{leg}] DD pass={dd['pass']} ({dd['fail_class']}) "
...
class VLMClient:
    ...
    def _boot(self):
        ...
        print("[P5.10] booting Jetson q8_0 server...", flush=True)
    ...
    def submit(self, frame_bgr, caption):
        ...
        for attempt in (0, 1):
            try:
                if self.be is None:
                    self._boot()
                path = f"/dev/shm/p510_{time.monotonic_ns()}.png"
                ...
                print(f"[P5.10] VLM call failed ({e!r}); "
...
def run_matrix(args) -> None:
    ...
    try:
        for clip in clips:
            ...
            if not todo:
                print(f"[P5.10] {clip}: all cells done, skipping", flush=True)
                continue
            ...
            if carry_file.exists():
                ...
                print(f"[P5.10] {clip}: carry pass loaded from cache", flush=True)
            else:
                ...
                print(f"[P5.10] {clip}: carry pass done "
        ...
        vlm.close()
    print(f"[P5.10] matrix done (vlm reboots: {vlm.reboots})", flush=True)
...
def preflight(bank_root: Path = BANK) -> None:
    ...
    for clip in CLIPS:
        ...
        assert not np.array_equal(a, b), f"{clip}: f75==f76 (dead feed)"
        vis = min(sum(g is not None for g in gtb[o]) for o in gtb)
        ov = max((iou(gtb[0][f], gtb[1][f]) if gtb[0][f] and gtb[1][f] else 0.0)
                 for f in range(N_FRAMES))
        print(f"[preflight] {clip}: 240 frames, phrases OK, min-visible {vis}/240, "
...
def _synth_clip(tmp: Path, jitter=0):
    """Synthetic 240-frame bank clip: noisy grey bg, moving white + blue rects,
...
def selfcheck() -> None:
    ...
    assert CAND_STRIDE == 8 and PROMPT_FRAME == 75
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert gtb[0][75] is not None and len(gtb[1]) == N_FRAMES
        ...
        tracks = carry_pass(lambda _r, b: mk[_seed_id(b, gtb)], frame_at,
                            {0: gtb[0][0], 1: gtb[1][0]})
        assert tracks[0][75] == gtb[0][75] and tracks[1][239] == gtb[1][239]
        ...
        junk = {0: {0: (0.0, 0.0, 5.0, 5.0), 75: (0.0, 0.0, 5.0, 5.0)},
                1: tracks[1]}
        ...
        r = rg_result("the white car", 0, 1, tracks, gtb, sub(gtb[0][75]), frame_at)
        assert r["pass"] and r["deliver_frame"] == 75 + round(4.6 * FPS) == 190
        ...
        r = rg_result("the white car", 0, 1, tracks, gtb, sub(gtb[0][75], 7.0),
                      frame_at)
        assert r["fail_class"] == "OVERRUN" and r["deliver_frame"] == 239, r
        ...
        r = rg_result("the white car", 0, 1, tracks, gtb, sub(gtb[1][75]), frame_at)
        ...
        drift = {0: {**tracks[0], **{f: (0.0, 0.0, 6.0, 6.0)
                                     for f in samples if f > 75}}, 1: tracks[1]}
        r = rg_result("the white car", 0, 1, drift, gtb, sub(gtb[0][75]), frame_at)
        ...
        # -- coverage
        c = coverage(tracks[0], gtb[0], 75)
        assert c["frac_lock"] == 1.0 and c["n"] == 165, c

        # -- overlays
        op = tmp / "ov.png"
        draw_overlay(frame_at(75), op, 75, "selfcheck", gtb[0][75], gtb[1][75],
                     delivered=gtb[0][75], vbox=gtb[1][75])
        img = cv2.imread(str(op))
        assert img is not None and img.shape == frame_at(75).shape

        # -- run_cell end-to-end (scripted carry + scripted VLM)
        res = run_cell(clip, "white", phrases, gtb, tracks,
                       sub(gtb[0][75]), frame_at, tmp / "cell")
        assert res["dd"]["pass"] and res["rg"]["pass"]
        for f in ("results.json", "overlay_dd_f0075.png", "overlay_vlm_f0075.png",
                  "overlay_rg_f0190.png", "overlay_end_f0239.png"):
    ...
    print("select_p510 selfcheck OK")
