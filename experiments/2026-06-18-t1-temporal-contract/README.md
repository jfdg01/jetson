...
(`experiments/2026-06-18-t0-cadence/README.md`, DECISIONS 2026-06-18T13:30).
...
numbers. Documented in `experiments/` + `RESULTS.md` + `DECISIONS.md` the same turn as the
gate pass.
...
- **2026-06-18 — T1a done → T1 GATE PASS.** `runners/sitl/clip_recorder.py`
...
the §6 suite. Reproduce: `.venv-ft/bin/python runners/sitl/clip_recorder.py --score <clip_dir>`.
...
```
.venv-ft/bin/python runners/sitl/oracle_bbox.py     # project_object + occluder geometry
.venv-ft/bin/python runners/sitl/clip_recorder.py   # reproducibility, stressors, scoring, discrimination
