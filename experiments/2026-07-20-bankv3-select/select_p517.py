"""P5.17 contract A/B select on the bank v3 lag-stress bank (Part V).

Forward-copy of P5.13's select_p513.py (rig contract IDENTICAL: same carry
pass, same delivery contracts, same fail taxonomy, same thresholds), diffed
ONLY in: bank source (v3, built by verdict_p517.py --build in this campaign's
runs/bank/), clip list (read from runs/bank_valid.json, n >= 25), and the
p513->p517 strings. Any A/B change vs P5.13 is therefore scene-attributable.
...
Scene source: the P5.17 bank v3 lag-stress bank (runs/bank/s<seed>, built by
verdict_p517.py --build from scenegen V3_BANK_PINNED), 300 frames @ 25 fps,
1280x720, per-frame GT for BOTH cars (id0 "the white car", id1 "the blue
car"). Exact dual GT makes selection scoring exact. The clip list is NOT
hard-coded: it is read from runs/bank_valid.json (written mechanically by the
build verdict), so only build-valid clips are scored.

Usage:
    .venv-ft/bin/python experiments/2026-07-20-bankv3-select/select_p517.py --selfcheck
    .venv-ft/bin/python experiments/2026-07-20-bankv3-select/select_p517.py --preflight
    .venv-ft/bin/python experiments/2026-07-20-bankv3-select/select_p517.py \
        --matrix
    # single clip / leg rerun (resumable; completed cells are skipped):
    ... --matrix --only s002 --legs white --out .../runs
"""
...
def load_clips() -> list[str]:
    """Clip list = the build verdict's output, never a hand-typed list."""
    f = HERE / "runs" / "bank_valid.json"
    assert f.exists(), \
        "runs/bank_valid.json missing -- run verdict_p517.py --build first"
    clips = json.loads(f.read_text())["valid"]
    assert len(clips) >= 25, \
        f"only {len(clips)} valid clips (< 25): bank build did not PASS"
    return clips
...
def run_cell(clip: str, leg: str, phrases: dict, gtb: dict, tracks: dict,
    ...
    print(f"[P5.17 {clip}_{leg}] DD pass={dd['pass']} ({dd['fail_class']}) "
...
class VLMClient:
    ...
    def _boot(self):
        ...
        print("[P5.17] booting Jetson q8_0 server...", flush=True)
    ...
    def submit(self, frame_bgr, caption):
        ...
        for attempt in (0, 1):
            try:
                if self.be is None:
                    self._boot()
                path = f"/dev/shm/p517_{time.monotonic_ns()}.png"
                ...
                print(f"[P5.17] VLM call failed ({e!r}); "
...
def run_matrix(args) -> None:
    ...
    clips = [c for c in load_clips() if args.only in (None, c)]
    ...
    try:
        for clip in clips:
            ...
            if not todo:
                print(f"[P5.17] {clip}: all cells done, skipping", flush=True)
                continue
            ...
            if carry_file.exists():
                ...
                print(f"[P5.17] {clip}: carry pass loaded from cache", flush=True)
            else:
                ...
                print(f"[P5.17] {clip}: carry pass done "
        ...
        vlm.close()
    print(f"[P5.17] matrix done (vlm reboots: {vlm.reboots})", flush=True)
...
def selfcheck() -> None:
    ...
    print("select_p517 selfcheck OK")
...
def main() -> None:
    ...
    ap.add_argument("--only", default=None, help="single clip, e.g. s002")
