#!/usr/bin/env python3
"""Mechanical verdict for P5.7 (sim scene-generator capability gate).

Reads runs/{seed101_A,seed202_B,seed303_C,seed101_D}/ produced by
runners/scenegen.py and prints the gate table + overall verdict. No judgment
calls live here -- thresholds are the pre-registered ones from the README.

Usage: .venv-ft/bin/python experiments/2026-07-17-sim-scenegen/verdict_p57.py
"""
import json
import sys
...
# (a hard bg ceiling would false-fail the white car whenever a control box lands
#  on the checkered start grid, which is legitimately white -- design smoke showed
#  bg 0.042 there)
BOTH_VIS_MIN = 0.80
FPS_MIN = 0.5
...
def main():
    ...
    for r in GATING:
        ...
        ok = g1 and g2 and g3 and g5
        all_ok &= ok
        rows.append((r, res["seed"], g1, g2, g3, g5, res["fps_wall"],
                     p["0"], p["1"], res["g3_both_visible_frac"]))
    ...
    print(f"{'run':<12} {'seed':<5} G1 G2 G3 G5  fps    pur0   pur1   bothvis")
    for r, s, g1, g2, g3, g5, fps, p0, p1, bv in rows:
        print(f"{r:<12} {s:<5} {int(g1)}  {int(g2)}  {int(g3)}  {int(g5)}  "
              f"{fps:<6.2f} {p0:<6.3f} {p1:<6.3f} {bv:.3f}")
    ...
    print(f"RQ-P5.7 OVERALL: {'YES' if all_ok else 'NO'} "
          "(YES iff G1,G2,G3,G5 on all 4 runs AND G4a AND G4b; "
