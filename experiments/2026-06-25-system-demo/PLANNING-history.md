...
dropped from the *viewer* (still fully in `experiments/`). The original plan is kept below for the record.

## ✅ BUILT — `experiments/2026-06-25-system-demo/` (2026-06-25)
The unified page is **done as a static 4-tab HTML** — open
`experiments/2026-06-25-system-demo/index.html`, no server. The Open-questions were resolved:
...
hook in `run_t3.run_loop` + `runners/sitl/closedloop_viz.py` (stage-4 viz, self-check
...
1. **Architecture + cadence (the "why two tiers")** — static panel: two-tier diagram +
   ...
   verbatim from `experiments/2026-06-24-t4-deployment/` + `experiments/2026-06-18-t0-cadence/`.
...
3. **Tracking tier — permanence (sim)** — embed the `permanence_viz` animation
   ...
   *Already built; embed `experiments/2026-06-24-t2-permanence/permanence.gif`.*
