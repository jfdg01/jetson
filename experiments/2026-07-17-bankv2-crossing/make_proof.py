#!/usr/bin/env python
"""P5.11 proof deliverables (reproducible from runs/*). Writes proof/*.png:
...
spec = importlib.util.spec_from_file_location("verdict_p511", HERE / "verdict_p511.py")
...
def fig_traces(cells):
    ...
    fig.suptitle("P5.11 bank v2: recorded GT-GT IoU per clip "
    ...
    for i in range(m.shape[0]):
        for j in range(m.shape[1]):
            ax.text(j, i, "P" if m[i, j] else "F", ha="center", va="center",
                    fontsize=8)
    ax.set_title(f"P5.11 gate grid ({int(m.all(axis=1).sum())}/{len(rows)} "
                 "cells pass all)")
    fig.tight_layout()
...
def fig_montage(cells):
    ...
    for ax in axes.flat[len(cells):]:
        ax.axis("off")
    fig.suptitle("P5.11 bank v2: crossing-peak overlay per clip "
