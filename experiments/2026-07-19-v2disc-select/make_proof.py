"""P5.13 proof deliverables, reproducible from runs/*/results.json + overlays.

  proof/p513_pass_grid.png       12 clips x {DD,RG} x {white,blue} pass/fail grid
  proof/p513_failclass.png       fail-class histogram per contract
  proof/p513_headline_dd_vs_rg.png  DD vs RG delivery overlays, headline cell
...
def pass_grid(cells):
    ...
    ax.set_title("P5.13 select on the bank v2.1 crossing bank: direct delivery (DD) "
                 "vs prompt-time re-ground (RG)")
    for s in ax.spines.values():
        s.set_visible(False)
    ax.tick_params(length=0)
    fig.tight_layout()
    fig.savefig(PROOF / "p513_pass_grid.png", dpi=160)
    plt.close(fig)
...
def failclass(cells):
    ...
    fig.tight_layout()
    fig.savefig(PROOF / "p513_failclass.png", dpi=160)
    plt.close(fig)
...
def headline(cells):
    ...
    for clip in CLIPS:
        for leg in LEGS:
            r = cells[(clip, leg)]
            if r and bool(r["dd"]["pass"]) != bool(r["rg"]["pass"]):
                pick = (clip, leg)
                break
        if pick:
            break
...
def main():
    ...
    failclass(cells)
    headline(cells)
    print(f"[make_proof] wrote {PROOF}/p513_pass_grid.png, p513_failclass.png, "
          f"p513_headline_dd_vs_rg.png")
