...
### RQ-R13.1 — On the Orin, does the deployed Qwen2-VL-2B Q8_0 beat OWLv2-base at referring-expression grounding?   [Part III · R-13]
- **Answer:** YES, decisively, against every non-oracle arm. 63.10% (277/439) vs D-full 25.74% (113/439), paired McNemar p=2.2e-24 deflated to 316 unique images; and still winning against the detector's *strongest* configuration D-phrase (noun phrase with adjectives), 63.10% vs 47.38%, p=2.3e-07. Both survive any reasonable multiplicity correction.
- **Why it matters:** The thesis premise — that a 2B VLM is the right tool for this task on a 15 W board — had never been measured against the cheap alternative. A repo-wide search for YOLO/OWLv2/GroundingDINO returned nothing outside prose. This is the first question an examiner asks, and it is now answered with a number instead of an assumption.
- **More:** `experiments/2026-07-21-detector-baseline/`

### RQ-R13.2 — How does OWLv2 split across D-full / D-phrase / D-head / D-oracle?   [Part III · R-13]
- **Answer:** The failure is **selection, not localisation**. D-oracle (best of top-10, chosen with GT) reaches 90.43%, beating the VLM itself (p=5.8e-25 in the detector's favour), and D-phrase recall@10 is 88.8% against 47.4% at top-1 — a 41.5 pp selection gap, with its second proposal already tying the VLM's top-1. Relational language actively hurts (D-full 21.6 pp below D-phrase), and adjectives carry the detector's whole contribution (D-phrase − D-head = 22.8 pp).
- **Why it matters:** The honest conclusion is not "OWLv2 is worse" but "the decomposed architecture needs a selection stage the 2026-06-14 campaign never costed" — and that stage is exactly the hard part, since the proposals are already there. That is a better and more defensible thesis sentence than the one previously in print.
- **More:** `experiments/2026-07-21-detector-baseline/`

### RQ-R13.3 — What does the accuracy difference cost in on-device latency and memory?   [Part III · R-13]
- **Answer:** It does not cost — it *saves*, and that reverses the original rationale. OWLv2 runs one forward in 263.5 ms median (p90 264.1, flat and input-independent) at 415.3 MB peak CUDA, against the VLM's 4319 ms wall: 16.4x cheaper per call, ~5x smaller. **The 2026-06-14 decision to close this fork "on latency grounds alone" was right for the wrong reason.** Caveat: the ratio compares one detector forward to one full generative anchor, and a decomposed system still needs the uncosted selection stage — if that stage is a VLM, the saving evaporates.
- **Why it matters:** A design decision justified by an unmeasured belief that turns out to be backwards is exactly what the lab-notebook rule exists to catch. The architecture survives, but on quality grounds, and the thesis now says so for a measured reason.
- **More:** `experiments/2026-07-21-detector-baseline/`
