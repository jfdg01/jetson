---
unit_id: model-capability-sweep-10
campaign: 2026-06-13-model-capability-sweep
title: "Meta-Llama-3.1-8B-Instruct Q4_K_M @ 15W locked"
status: TODO
created: 2026-06-13
---

# Unit 10: Meta-Llama-3.1-8B-Instruct Q4_K_M @ 15 W locked
...
## Objective
Model #10 of the sweep — Tier C heavy, family Llama-3.1, 8 B. Largest dense model expected to fit; the memory-wall probe (RQ2 / H3).
...
- [ ] `df -h /` shows ample free space (need ~5 GB; NVMe had ~196 GB free).
...
| Params | 8 B |
| Quant | Q4_K_M |
| GGUF repo | `bartowski/Meta-Llama-3.1-8B-Instruct-GGUF` (Hugging Face) |
...
ssh jetson 'nohup tegrastats --interval 1000 --logfile /tmp/msweep10_tegra.log >/dev/null 2>&1 & echo started pid $!'

# 1. acquire model onto NVMe via wget (curl/hf-cli not installed), then record sha256
ssh jetson 'wget -c -O ~/models/Meta-Llama-3.1-8B-Instruct-Q4_K_M.gguf \
   "https://huggingface.co/bartowski/Meta-Llama-3.1-8B-Instruct-GGUF/resolve/main/Meta-Llama-3.1-8B-Instruct-Q4_K_M.gguf"'
ssh jetson 'sha256sum ~/models/Meta-Llama-3.1-8B-Instruct-Q4_K_M.gguf'   # record the hash + the resolved repo revision
...
ssh jetson 'export LD_LIBRARY_PATH=~/llama.cpp/build/bin:/usr/local/cuda/lib64; \
   ~/llama.cpp/build/bin/llama-bench -m ~/models/Meta-Llama-3.1-8B-Instruct-Q4_K_M.gguf -ngl 99 -p 512 -n 128 -r 5 -o csv' \
   | tee /tmp/msweep10_bench.csv
# sustained decode droop check
ssh jetson 'export LD_LIBRARY_PATH=~/llama.cpp/build/bin:/usr/local/cuda/lib64; \
   ~/llama.cpp/build/bin/llama-bench -m ~/models/Meta-Llama-3.1-8B-Instruct-Q4_K_M.gguf -ngl 99 -n 512 -r 3'

# 4. TTFT — single timed generation (llama-cli prints first-token + per-token timings)
ssh jetson 'export LD_LIBRARY_PATH=~/llama.cpp/build/bin:/usr/local/cuda/lib64; \
   ~/llama.cpp/build/bin/llama-cli -m ~/models/Meta-Llama-3.1-8B-Instruct-Q4_K_M.gguf -ngl 99 -c 4096 -n 128 \
...
ssh jetson 'free -m; cat /proc/swaps; tail -n 5 /tmp/msweep10_tegra.log'

# 6. stop the power log
ssh jetson 'pkill -f tegrastats || true'

# 7. pull artifacts back into the repo
scp jetson:/tmp/msweep10_tegra.log experiments/raw/2026-06-13_msweep10_tegra.log
cp /tmp/msweep10_bench.csv experiments/raw/2026-06-13_msweep10_bench.csv
```

## Output contract  (all of these, before you stop)
- [ ] `experiments/raw/2026-06-13_msweep10_tegra.log` and `experiments/raw/2026-06-13_msweep10_bench.csv` saved.
- [ ] Detail block appended to `experiments/2026-06-13-model-capability-sweep.md` (new
      "### Unit 10 — Meta-Llama-3.1-8B-Instruct" subsection under a "## Results" heading) with EVERY mandatory
...
- [ ] Fit note for this tier: TIGHTEST fit (~4.6 GB weights + KV at 4096) — the most likely OOM/swap point.
- [ ] No thermal throttle, or throttle explicitly noted if it occurred.

## Failure handling  (failures are data)
OOM/swap is the EXPECTED, valuable result here (H3). Record peak footprint, whether zram was hit, and the error; set FAILED if it cannot complete at n_ctx=4096. The n_ctx context sub-sweep is a separate follow-up unit, not this one.
