---
unit_id: model-capability-sweep-08
campaign: 2026-06-13-model-capability-sweep
title: "Mistral-7B-Instruct-v0.3 Q4_K_M @ 15W locked"
status: TODO
created: 2026-06-13
---

# Unit 08: Mistral-7B-Instruct-v0.3 Q4_K_M @ 15 W locked
...
## Objective
Model #08 of the sweep — Tier C heavy, family Mistral, 7.2 B. First of the heavy tier: 7 B tight-fit point.
...
| Params | 7.2 B |
| Quant | Q4_K_M |
| GGUF repo | `bartowski/Mistral-7B-Instruct-v0.3-GGUF` (Hugging Face) |
...
ssh jetson 'nohup tegrastats --interval 1000 --logfile /tmp/msweep08_tegra.log >/dev/null 2>&1 & echo started pid $!'

# 1. acquire model onto NVMe via wget (curl/hf-cli not installed), then record sha256
ssh jetson 'wget -c -O ~/models/Mistral-7B-Instruct-v0.3-Q4_K_M.gguf \
   "https://huggingface.co/bartowski/Mistral-7B-Instruct-v0.3-GGUF/resolve/main/Mistral-7B-Instruct-v0.3-Q4_K_M.gguf"'
ssh jetson 'sha256sum ~/models/Mistral-7B-Instruct-v0.3-Q4_K_M.gguf'   # record the hash + the resolved repo revision
...
ssh jetson 'export LD_LIBRARY_PATH=~/llama.cpp/build/bin:/usr/local/cuda/lib64; \
   ~/llama.cpp/build/bin/llama-bench -m ~/models/Mistral-7B-Instruct-v0.3-Q4_K_M.gguf -ngl 99 -p 512 -n 128 -r 5 -o csv' \
   | tee /tmp/msweep08_bench.csv
# sustained decode droop check
ssh jetson 'export LD_LIBRARY_PATH=~/llama.cpp/build/bin:/usr/local/cuda/lib64; \
   ~/llama.cpp/build/bin/llama-bench -m ~/models/Mistral-7B-Instruct-v0.3-Q4_K_M.gguf -ngl 99 -n 512 -r 3'

# 4. TTFT — single timed generation (llama-cli prints first-token + per-token timings)
ssh jetson 'export LD_LIBRARY_PATH=~/llama.cpp/build/bin:/usr/local/cuda/lib64; \
   ~/llama.cpp/build/bin/llama-cli -m ~/models/Mistral-7B-Instruct-v0.3-Q4_K_M.gguf -ngl 99 -c 4096 -n 128 \
...
ssh jetson 'free -m; cat /proc/swaps; tail -n 5 /tmp/msweep08_tegra.log'

# 6. stop the power log
ssh jetson 'pkill -f tegrastats || true'

# 7. pull artifacts back into the repo
scp jetson:/tmp/msweep08_tegra.log experiments/raw/2026-06-13_msweep08_tegra.log
cp /tmp/msweep08_bench.csv experiments/raw/2026-06-13_msweep08_bench.csv
```

## Output contract  (all of these, before you stop)
- [ ] `experiments/raw/2026-06-13_msweep08_tegra.log` and `experiments/raw/2026-06-13_msweep08_bench.csv` saved.
- [ ] Detail block appended to `experiments/2026-06-13-model-capability-sweep.md` (new
      "### Unit 08 — Mistral-7B-Instruct-v0.3" subsection under a "## Results" heading) with EVERY mandatory
...
- [ ] Fit note for this tier: TIGHT fit (~4.2 GB weights + KV) — watch zram swap.
- [ ] No thermal throttle, or throttle explicitly noted if it occurred.

## Failure handling  (failures are data)
OOM or zram thrash is POSSIBLE here and is a VALID result (tests H3). If it OOMs/thrashes, record the peak footprint, whether swap was hit, and the error — do NOT shrink n_ctx to force success (that is the separate sub-sweep).
