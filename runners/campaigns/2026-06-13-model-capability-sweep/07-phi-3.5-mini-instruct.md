---
unit_id: model-capability-sweep-07
campaign: 2026-06-13-model-capability-sweep
title: "Phi-3.5-mini-instruct Q4_K_M @ 15W locked"
status: TODO
created: 2026-06-13
---

# Unit 07: Phi-3.5-mini-instruct Q4_K_M @ 15 W locked
...
## Objective
Model #07 of the sweep — Tier B sweet-spot, family Phi-3.5, 3.8 B. Different architecture (Phi-3.5) at the top of the sweet-spot tier (~4 B).
...
- [ ] `df -h /` shows ample free space (need ~3 GB; NVMe had ~196 GB free).
...
| Params | 3.8 B |
| Quant | Q4_K_M |
| GGUF repo | `bartowski/Phi-3.5-mini-instruct-GGUF` (Hugging Face) |
| Revision | record the resolved commit hash at download |
| File | `~/models/Phi-3.5-mini-instruct-Q4_K_M.gguf` |
| SHA256 | <fill at acquisition; record in the result block> |
| Expected size | ~2282 MB |
...
ssh jetson 'nohup tegrastats --interval 1000 --logfile /tmp/msweep07_tegra.log >/dev/null 2>&1 & echo started pid $!'

# 1. acquire model onto NVMe via wget (curl/hf-cli not installed), then record sha256
ssh jetson 'wget -c -O ~/models/Phi-3.5-mini-instruct-Q4_K_M.gguf \
   "https://huggingface.co/bartowski/Phi-3.5-mini-instruct-GGUF/resolve/main/Phi-3.5-mini-instruct-Q4_K_M.gguf"'
ssh jetson 'sha256sum ~/models/Phi-3.5-mini-instruct-Q4_K_M.gguf'   # record the hash + the resolved repo revision
...
ssh jetson 'export LD_LIBRARY_PATH=~/llama.cpp/build/bin:/usr/local/cuda/lib64; \
   ~/llama.cpp/build/bin/llama-bench -m ~/models/Phi-3.5-mini-instruct-Q4_K_M.gguf -ngl 99 -p 512 -n 128 -r 5 -o csv' \
   | tee /tmp/msweep07_bench.csv
# sustained decode droop check
ssh jetson 'export LD_LIBRARY_PATH=~/llama.cpp/build/bin:/usr/local/cuda/lib64; \
   ~/llama.cpp/build/bin/llama-bench -m ~/models/Phi-3.5-mini-instruct-Q4_K_M.gguf -ngl 99 -n 512 -r 3'

# 4. TTFT — single timed generation (llama-cli prints first-token + per-token timings)
ssh jetson 'export LD_LIBRARY_PATH=~/llama.cpp/build/bin:/usr/local/cuda/lib64; \
   ~/llama.cpp/build/bin/llama-cli -m ~/models/Phi-3.5-mini-instruct-Q4_K_M.gguf -ngl 99 -c 4096 -n 128 \
...
ssh jetson 'free -m; cat /proc/swaps; tail -n 5 /tmp/msweep07_tegra.log'

# 6. stop the power log
ssh jetson 'pkill -f tegrastats || true'

# 7. pull artifacts back into the repo
scp jetson:/tmp/msweep07_tegra.log experiments/raw/2026-06-13_msweep07_tegra.log
cp /tmp/msweep07_bench.csv experiments/raw/2026-06-13_msweep07_bench.csv
```

## Output contract  (all of these, before you stop)
- [ ] `experiments/raw/2026-06-13_msweep07_tegra.log` and `experiments/raw/2026-06-13_msweep07_bench.csv` saved.
- [ ] Detail block appended to `experiments/2026-06-13-model-capability-sweep.md` (new
      "### Unit 07 — Phi-3.5-mini-instruct" subsection under a "## Results" heading) with EVERY mandatory
