---
unit_id: model-capability-sweep-04
campaign: 2026-06-13-model-capability-sweep
title: "Gemma-2-2B-it Q4_K_M @ 15W locked"
status: TODO
created: 2026-06-13
---

# Unit 04: Gemma-2-2B-it Q4_K_M @ 15 W locked
...
## Objective
Model #04 of the sweep — Tier B sweet-spot, family Gemma-2, 2.6 B. Different architecture (Gemma-2, sliding-window attention) at the 2 B sweet spot.
...
| Params | 2.6 B |
| Quant | Q4_K_M |
| GGUF repo | `bartowski/gemma-2-2b-it-GGUF` (Hugging Face) |
...
ssh jetson 'nohup tegrastats --interval 1000 --logfile /tmp/msweep04_tegra.log >/dev/null 2>&1 & echo started pid $!'

# 1. acquire model onto NVMe via wget (curl/hf-cli not installed), then record sha256
ssh jetson 'wget -c -O ~/models/gemma-2-2b-it-Q4_K_M.gguf \
   "https://huggingface.co/bartowski/gemma-2-2b-it-GGUF/resolve/main/gemma-2-2b-it-Q4_K_M.gguf"'
ssh jetson 'sha256sum ~/models/gemma-2-2b-it-Q4_K_M.gguf'   # record the hash + the resolved repo revision
...
ssh jetson 'export LD_LIBRARY_PATH=~/llama.cpp/build/bin:/usr/local/cuda/lib64; \
   ~/llama.cpp/build/bin/llama-bench -m ~/models/gemma-2-2b-it-Q4_K_M.gguf -ngl 99 -p 512 -n 128 -r 5 -o csv' \
   | tee /tmp/msweep04_bench.csv
# sustained decode droop check
ssh jetson 'export LD_LIBRARY_PATH=~/llama.cpp/build/bin:/usr/local/cuda/lib64; \
   ~/llama.cpp/build/bin/llama-bench -m ~/models/gemma-2-2b-it-Q4_K_M.gguf -ngl 99 -n 512 -r 3'

# 4. TTFT — single timed generation (llama-cli prints first-token + per-token timings)
ssh jetson 'export LD_LIBRARY_PATH=~/llama.cpp/build/bin:/usr/local/cuda/lib64; \
   ~/llama.cpp/build/bin/llama-cli -m ~/models/gemma-2-2b-it-Q4_K_M.gguf -ngl 99 -c 4096 -n 128 \
...
ssh jetson 'free -m; cat /proc/swaps; tail -n 5 /tmp/msweep04_tegra.log'

# 6. stop the power log
ssh jetson 'pkill -f tegrastats || true'

# 7. pull artifacts back into the repo
scp jetson:/tmp/msweep04_tegra.log experiments/raw/2026-06-13_msweep04_tegra.log
cp /tmp/msweep04_bench.csv experiments/raw/2026-06-13_msweep04_bench.csv
```

## Output contract  (all of these, before you stop)
- [ ] `experiments/raw/2026-06-13_msweep04_tegra.log` and `experiments/raw/2026-06-13_msweep04_bench.csv` saved.
- [ ] Detail block appended to `experiments/2026-06-13-model-capability-sweep.md` (new
      "### Unit 04 — Gemma-2-2B-it" subsection under a "## Results" heading) with EVERY mandatory
...
- [ ] Fit note for this tier: comfortable fit.
- [ ] No thermal throttle, or throttle explicitly noted if it occurred.

## Failure handling  (failures are data)
OOM not expected; note Gemma-2 sliding-window attention may affect KV behaviour — record anything unusual.
