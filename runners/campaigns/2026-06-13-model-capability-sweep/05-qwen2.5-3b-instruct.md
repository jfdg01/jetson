---
unit_id: model-capability-sweep-05
campaign: 2026-06-13-model-capability-sweep
title: "Qwen2.5-3B-Instruct Q4_K_M @ 15W locked"
status: TODO
created: 2026-06-13
---

# Unit 05: Qwen2.5-3B-Instruct Q4_K_M @ 15 W locked
...
## Objective
Model #05 of the sweep — Tier B sweet-spot, family Qwen2.5, 3 B. Qwen within-family scaling point 3 of 4.
...
| Params | 3 B |
| Quant | Q4_K_M |
| GGUF repo | `bartowski/Qwen2.5-3B-Instruct-GGUF` (Hugging Face) |
...
ssh jetson 'nohup tegrastats --interval 1000 --logfile /tmp/msweep05_tegra.log >/dev/null 2>&1 & echo started pid $!'
...
ssh jetson 'sha256sum ~/models/Qwen2.5-3B-Instruct-Q4_K_M.gguf'   # record the hash + the resolved repo revision
...
ssh jetson 'export LD_LIBRARY_PATH=~/llama.cpp/build/bin:/usr/local/cuda/lib64; \
   ~/llama.cpp/build/bin/llama-bench -m ~/models/Qwen2.5-3B-Instruct-Q4_K_M.gguf -ngl 99 -p 512 -n 128 -r 5 -o csv' \
   | tee /tmp/msweep05_bench.csv
# sustained decode droop check
ssh jetson 'export LD_LIBRARY_PATH=~/llama.cpp/build/bin:/usr/local/cuda/lib64; \
   ~/llama.cpp/build/bin/llama-bench -m ~/models/Qwen2.5-3B-Instruct-Q4_K_M.gguf -ngl 99 -n 512 -r 3'

# 4. TTFT — single timed generation (llama-cli prints first-token + per-token timings)
ssh jetson 'export LD_LIBRARY_PATH=~/llama.cpp/build/bin:/usr/local/cuda/lib64; \
   ~/llama.cpp/build/bin/llama-cli -m ~/models/Qwen2.5-3B-Instruct-Q4_K_M.gguf -ngl 99 -c 4096 -n 128 \
...
scp jetson:/tmp/msweep05_tegra.log experiments/raw/2026-06-13_msweep05_tegra.log
cp /tmp/msweep05_bench.csv experiments/raw/2026-06-13_msweep05_bench.csv
```

## Output contract  (all of these, before you stop)
- [ ] `experiments/raw/2026-06-13_msweep05_tegra.log` and `experiments/raw/2026-06-13_msweep05_bench.csv` saved.
- [ ] Detail block appended to `experiments/2026-06-13-model-capability-sweep.md` (new
      "### Unit 05 — Qwen2.5-3B-Instruct" subsection under a "## Results" heading) with EVERY mandatory
