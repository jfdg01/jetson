---
unit_id: model-capability-sweep-09
campaign: 2026-06-13-model-capability-sweep
title: "Qwen2.5-7B-Instruct Q4_K_M @ 15W locked"
status: TODO
created: 2026-06-13
---

# Unit 09: Qwen2.5-7B-Instruct Q4_K_M @ 15 W locked
...
## Objective
Model #09 of the sweep — Tier C heavy, family Qwen2.5, 7.6 B. Qwen within-family scaling point 4 of 4 — completes the controlled spine at the heavy end.
...
| Params | 7.6 B |
| Quant | Q4_K_M |
| GGUF repo | `bartowski/Qwen2.5-7B-Instruct-GGUF` (Hugging Face) |
...
ssh jetson 'nohup tegrastats --interval 1000 --logfile /tmp/msweep09_tegra.log >/dev/null 2>&1 & echo started pid $!'

# 1. acquire model onto NVMe via wget (curl/hf-cli not installed), then record sha256
ssh jetson 'wget -c -O ~/models/Qwen2.5-7B-Instruct-Q4_K_M.gguf \
   "https://huggingface.co/bartowski/Qwen2.5-7B-Instruct-GGUF/resolve/main/Qwen2.5-7B-Instruct-Q4_K_M.gguf"'
ssh jetson 'sha256sum ~/models/Qwen2.5-7B-Instruct-Q4_K_M.gguf'   # record the hash + the resolved repo revision
...
ssh jetson 'export LD_LIBRARY_PATH=~/llama.cpp/build/bin:/usr/local/cuda/lib64; \
   ~/llama.cpp/build/bin/llama-bench -m ~/models/Qwen2.5-7B-Instruct-Q4_K_M.gguf -ngl 99 -p 512 -n 128 -r 5 -o csv' \
   | tee /tmp/msweep09_bench.csv
# sustained decode droop check
ssh jetson 'export LD_LIBRARY_PATH=~/llama.cpp/build/bin:/usr/local/cuda/lib64; \
   ~/llama.cpp/build/bin/llama-bench -m ~/models/Qwen2.5-7B-Instruct-Q4_K_M.gguf -ngl 99 -n 512 -r 3'

# 4. TTFT — single timed generation (llama-cli prints first-token + per-token timings)
ssh jetson 'export LD_LIBRARY_PATH=~/llama.cpp/build/bin:/usr/local/cuda/lib64; \
   ~/llama.cpp/build/bin/llama-cli -m ~/models/Qwen2.5-7B-Instruct-Q4_K_M.gguf -ngl 99 -c 4096 -n 128 \
...
ssh jetson 'free -m; cat /proc/swaps; tail -n 5 /tmp/msweep09_tegra.log'

# 6. stop the power log
ssh jetson 'pkill -f tegrastats || true'

# 7. pull artifacts back into the repo
scp jetson:/tmp/msweep09_tegra.log experiments/raw/2026-06-13_msweep09_tegra.log
cp /tmp/msweep09_bench.csv experiments/raw/2026-06-13_msweep09_bench.csv
```

## Output contract  (all of these, before you stop)
- [ ] `experiments/raw/2026-06-13_msweep09_tegra.log` and `experiments/raw/2026-06-13_msweep09_bench.csv` saved.
- [ ] Detail block appended to `experiments/2026-06-13-model-capability-sweep.md` (new
      "### Unit 09 — Qwen2.5-7B-Instruct" subsection under a "## Results" heading) with EVERY mandatory
...
- [ ] Fit note for this tier: TIGHT fit — watch swap.
- [ ] No thermal throttle, or throttle explicitly noted if it occurred.

## Failure handling  (failures are data)
OOM/swap possible (H3); record it as a result. Do NOT shrink n_ctx to force a pass.
