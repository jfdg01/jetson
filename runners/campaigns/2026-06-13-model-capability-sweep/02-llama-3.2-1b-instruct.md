---
unit_id: model-capability-sweep-02
campaign: 2026-06-13-model-capability-sweep
title: "Llama-3.2-1B-Instruct Q4_K_M @ 15W locked"
status: TODO
created: 2026-06-13
---

# Unit 02: Llama-3.2-1B-Instruct Q4_K_M @ 15 W locked
...
## Objective
Model #02 of the sweep — Tier A ultralight, family Llama-3.2, 1 B. 1 B cross-family point (Llama vs Qwen at similar scale).
...
| Params | 1 B |
| Quant | Q4_K_M |
| GGUF repo | `bartowski/Llama-3.2-1B-Instruct-GGUF` (Hugging Face) |
...
ssh jetson 'nohup tegrastats --interval 1000 --logfile /tmp/msweep02_tegra.log >/dev/null 2>&1 & echo started pid $!'

# 1. acquire model onto NVMe via wget (curl/hf-cli not installed), then record sha256
ssh jetson 'wget -c -O ~/models/Llama-3.2-1B-Instruct-Q4_K_M.gguf \
   "https://huggingface.co/bartowski/Llama-3.2-1B-Instruct-GGUF/resolve/main/Llama-3.2-1B-Instruct-Q4_K_M.gguf"'
ssh jetson 'sha256sum ~/models/Llama-3.2-1B-Instruct-Q4_K_M.gguf'   # record the hash + the resolved repo revision
...
ssh jetson 'export LD_LIBRARY_PATH=~/llama.cpp/build/bin:/usr/local/cuda/lib64; \
   ~/llama.cpp/build/bin/llama-bench -m ~/models/Llama-3.2-1B-Instruct-Q4_K_M.gguf -ngl 99 -p 512 -n 128 -r 5 -o csv' \
   | tee /tmp/msweep02_bench.csv
# sustained decode droop check
ssh jetson 'export LD_LIBRARY_PATH=~/llama.cpp/build/bin:/usr/local/cuda/lib64; \
   ~/llama.cpp/build/bin/llama-bench -m ~/models/Llama-3.2-1B-Instruct-Q4_K_M.gguf -ngl 99 -n 512 -r 3'

# 4. TTFT — single timed generation (llama-cli prints first-token + per-token timings)
ssh jetson 'export LD_LIBRARY_PATH=~/llama.cpp/build/bin:/usr/local/cuda/lib64; \
   ~/llama.cpp/build/bin/llama-cli -m ~/models/Llama-3.2-1B-Instruct-Q4_K_M.gguf -ngl 99 -c 4096 -n 128 \
...
ssh jetson 'free -m; cat /proc/swaps; tail -n 5 /tmp/msweep02_tegra.log'

# 6. stop the power log
ssh jetson 'pkill -f tegrastats || true'

# 7. pull artifacts back into the repo
scp jetson:/tmp/msweep02_tegra.log experiments/raw/2026-06-13_msweep02_tegra.log
cp /tmp/msweep02_bench.csv experiments/raw/2026-06-13_msweep02_bench.csv
```

## Output contract  (all of these, before you stop)
- [ ] `experiments/raw/2026-06-13_msweep02_tegra.log` and `experiments/raw/2026-06-13_msweep02_bench.csv` saved.
- [ ] Detail block appended to `experiments/2026-06-13-model-capability-sweep.md` (new
      "### Unit 02 — Llama-3.2-1B-Instruct" subsection under a "## Results" heading) with EVERY mandatory
