---
unit_id: model-capability-sweep-03
campaign: 2026-06-13-model-capability-sweep
title: "Qwen2.5-1.5B-Instruct Q4_K_M @ 15W locked"
status: TODO
created: 2026-06-13
---

# Unit 03: Qwen2.5-1.5B-Instruct Q4_K_M @ 15 W locked
...
## Objective
Model #03 of the sweep — Tier A light, family Qwen2.5, 1.5 B. Qwen within-family scaling point 2 of 4 (0.5 to 1.5 to 3 to 7.6 B spine).
...
| Params | 1.5 B |
| Quant | Q4_K_M |
| GGUF repo | `bartowski/Qwen2.5-1.5B-Instruct-GGUF` (Hugging Face) |
...
ssh jetson 'nohup tegrastats --interval 1000 --logfile /tmp/msweep03_tegra.log >/dev/null 2>&1 & echo started pid $!'

# 1. acquire model onto NVMe via wget (curl/hf-cli not installed), then record sha256
ssh jetson 'wget -c -O ~/models/Qwen2.5-1.5B-Instruct-Q4_K_M.gguf \
   "https://huggingface.co/bartowski/Qwen2.5-1.5B-Instruct-GGUF/resolve/main/Qwen2.5-1.5B-Instruct-Q4_K_M.gguf"'
ssh jetson 'sha256sum ~/models/Qwen2.5-1.5B-Instruct-Q4_K_M.gguf'   # record the hash + the resolved repo revision
...
ssh jetson 'export LD_LIBRARY_PATH=~/llama.cpp/build/bin:/usr/local/cuda/lib64; \
   ~/llama.cpp/build/bin/llama-bench -m ~/models/Qwen2.5-1.5B-Instruct-Q4_K_M.gguf -ngl 99 -p 512 -n 128 -r 5 -o csv' \
   | tee /tmp/msweep03_bench.csv
# sustained decode droop check
ssh jetson 'export LD_LIBRARY_PATH=~/llama.cpp/build/bin:/usr/local/cuda/lib64; \
   ~/llama.cpp/build/bin/llama-bench -m ~/models/Qwen2.5-1.5B-Instruct-Q4_K_M.gguf -ngl 99 -n 512 -r 3'

# 4. TTFT — single timed generation (llama-cli prints first-token + per-token timings)
ssh jetson 'export LD_LIBRARY_PATH=~/llama.cpp/build/bin:/usr/local/cuda/lib64; \
   ~/llama.cpp/build/bin/llama-cli -m ~/models/Qwen2.5-1.5B-Instruct-Q4_K_M.gguf -ngl 99 -c 4096 -n 128 \
...
ssh jetson 'free -m; cat /proc/swaps; tail -n 5 /tmp/msweep03_tegra.log'

# 6. stop the power log
ssh jetson 'pkill -f tegrastats || true'

# 7. pull artifacts back into the repo
scp jetson:/tmp/msweep03_tegra.log experiments/raw/2026-06-13_msweep03_tegra.log
cp /tmp/msweep03_bench.csv experiments/raw/2026-06-13_msweep03_bench.csv
```

## Output contract  (all of these, before you stop)
- [ ] `experiments/raw/2026-06-13_msweep03_tegra.log` and `experiments/raw/2026-06-13_msweep03_bench.csv` saved.
- [ ] Detail block appended to `experiments/2026-06-13-model-capability-sweep.md` (new
      "### Unit 03 — Qwen2.5-1.5B-Instruct" subsection under a "## Results" heading) with EVERY mandatory
...
- [ ] Fit note for this tier: comfortable headroom.
