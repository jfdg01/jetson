...
`experiments/run_grounding_probe.py` and `run_phase_c.py`.
...
6. Run Phase A probe: `python experiments/run_grounding_probe.py --vlm-model ~/smolvlm_ft_q8_0.gguf`
...
| Quantization | Q8_0 |
| Phase A probe set | Same VisDrone frames as Stage 1 (`results/2026-06-14-stage1-baseline/raw/grounding_probe_*.jpg`) |
| Phase C world | `experiments/sitl/worlds/phase_c.sdf` (unchanged) |
...
- [ ] `experiments/run_stage2_finetune.py` — training loop
- [ ] `experiments/eval_ft_grounding.py` — HF-checkpoint evaluation (before GGUF conversion)
...
| File | Contents |
|------|----------|
| `results/stage2-finetune/train-log.md` | Per-epoch loss + eval IoU, training time, GPU memory |
| `results/stage2-finetune/phase-a-rerun.md` | Phase A grounding probe results with FT model |
| `results/stage2-finetune/phase-c-rerun.md` | Phase C Branch-2 re-run with FT model |
| `results/stage2-finetune/raw/train_loss.csv` | Raw loss curve |
| `results/stage2-finetune/raw/eval_iou.csv` | Per-sample IoU on val set |
