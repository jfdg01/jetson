...
**Builds on:** Stage 3 (`results/stage3-refcoco-finetune/`), which PASSED gate G2
...
python experiments/run_stage4_finetune.py --dry-run                      # verify model+LoRA+collate+JSON resolve
python experiments/run_stage4_finetune.py --init-from ./smolvlm_ft3 --epochs 3 --lr 1e-4   # ~1h
python experiments/run_stage4_finetune.py --eval-only ./smolvlm_ft4      # re-eval merged ckpt
```
