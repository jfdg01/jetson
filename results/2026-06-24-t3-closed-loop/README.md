...
(`results/2026-06-24-t2-permanence/`, DECISIONS 2026-06-24).
...
## Scenario (`experiments/run_t3.py`)
...
### Kinematic closed loop (deterministic A/B — `.venv-ft/bin/python experiments/run_t3.py`)
...
### Live ArduCopter SITL (real flight dynamics — `.venv/bin/python experiments/run_t3.py --live`)
...
- **Decision:** Deliver T3 as a **two-policy closed-loop A/B** (memoryless vs T2 re-ID) on
  ...
  runner (`experiments/run_t3.py`, ~280 lines) reusing oracle_bbox + bytetrack + cascade_pid
...
```
.venv-ft/bin/python experiments/run_t3.py          # deterministic kinematic self-checks (3 PASS)
.venv/bin/python    experiments/run_t3.py --live    # real ArduCopter SITL, one flight per policy
```
