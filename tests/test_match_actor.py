...
class FakeWorld:
    ...
    def get_actors(self):
        return type("L", (), {"filter": lambda _s, _p: self._a})()
...
def test_tight_box_missing_origin_still_matches():
    ...
    assert ui.match_actor(FakeWorld([truck]), CAM, tight) is truck
...
def test_box_on_empty_asphalt_is_a_drift():
    ...
    assert ui.match_actor(FakeWorld([truck]),
                          CAM, (far[0] - 20, far[1] - 20, far[0] + 20, far[1] + 20)) is None
...
def test_nearest_neighbour_does_not_steal_the_lock():
    ...
    assert ui.match_actor(FakeWorld([a, b]), CAM, ui.actor_box(a, CAM)) is a
    assert ui.match_actor(FakeWorld([a, b]), CAM, ui.actor_box(b, CAM)) is b
